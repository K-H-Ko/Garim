...
-- 3) subscriptions: 30-day period, auto-renewal, cancellation reservation, carry-over tracking
...
COMMENT ON COLUMN subscriptions.auto_renew IS '자동결제 여부 - false이면 자동결제 대상 제외. 업그레이드로 밀린 하위 구독은 false 처리';
...
COMMENT ON COLUMN subscriptions.upgraded_at IS '업그레이드 처리 일시 - 이 구독이 상위 플랜으로 밀려난 시각';
COMMENT ON COLUMN subscriptions.superseded_by_subscription_id IS '상위 구독 ID - 이 구독보다 우선 적용되는 상위 구독';
COMMENT ON COLUMN subscriptions.carried_over_days IS '이월 일수 - 업그레이드 시 상위 플랜 종료 후 이어서 적용될 하위 플랜 잔여 일수';
COMMENT ON COLUMN subscriptions.original_period_end IS '원래 기간 종료 - 업그레이드 전 하위 구독의 기존 종료일';
...
CREATE TABLE IF NOT EXISTS subscription_plan_changes (
    ...
    CONSTRAINT chk_subscription_plan_changes_remaining_days CHECK (remaining_days >= 0),
...
COMMENT ON COLUMN subscription_plan_changes.remaining_days IS '잔여 일수 - 업그레이드 이월 표시용 일수';
COMMENT ON COLUMN subscription_plan_changes.remaining_seconds IS '잔여 초 - 업그레이드 이월 계산용 초 단위 값';
COMMENT ON COLUMN subscription_plan_changes.from_subscription_original_end IS '기존 구독 원래 종료일 - 업그레이드 전 종료일';
COMMENT ON COLUMN subscription_plan_changes.from_subscription_new_end IS '기존 구독 새 종료일 - 이월 반영 후 종료일';
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_subscriptions_carried_over_days_non_negative') THEN
        ALTER TABLE subscriptions
            ADD CONSTRAINT ck_subscriptions_carried_over_days_non_negative
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_subscriptions_superseded_by') THEN
        ALTER TABLE subscriptions
            ADD CONSTRAINT fk_subscriptions_superseded_by
            FOREIGN KEY (superseded_by_subscription_id) REFERENCES subscriptions(subscription_id) ON DELETE SET NULL;
    END IF;
...
ON subscriptions (superseded_by_subscription_id);

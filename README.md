# 🎥 GARIM (가림) - 딥러닝 기반 개인정보 독싱(Doxing) 방지 플랫폼

본 프로젝트는 이미지/영상/음성 내의 **개인 식별 정보(PII)**를 자동으로 탐지하고 완벽하게 마스킹(Inpainting) 및 묵음 처리(Beeping)하여, **'퍼즐 독싱(Doxing)'**과 같은 개인정보 유출을 방지하기 위한 AI 서비스입니다.
...
4. **UX 최적화**
   ...
   - 영상 전체 마스킹 인코딩 대신 PII 발생 전후 3초(총 6초)만 샘플링하여 실시간 미리보기를 제공합니다.
